...
const WeatherCard = ({currData, pollData}:any) => {
            ...
            > 
                <Flex alignItems='center' justifyContent='center' gap='15px'>
                    ...
                    <VStack spacing={0} alignItems='flex-start' lineHeight='25px'>
                        ...
                        <Text fontSize='16px'>PM 2.5 : <b>{pollData?.list[0].components.pm2_5}</b></Text>
                    </VStack>
                </Flex>

                <VStack spacing={0} align='flex-start'>
                    <Text fontSize='36px' align='center' fontWeight='500'>{pollData?.list[0].main.aqi} <Badge colorScheme='yellow' fontSize='14px' fontWeight='400'>AQI</Badge></Text>
                ...
                <VStack w='100%' gap='10px' alignItems='space-between' p={4} borderRadius={'15px'} boxShadow='xl'>
                    ...
                    <Tooltip label={`${pollData?.list[0].main.aqi}AQI`} hasArrow>
                        <Progress value={pollData?.list[0].main.aqi} min={0} max={500} colorScheme={progress < 150? 'green' :  progress>150  ? 'yellow' : progress > 300 ? 'tomato' : 'teal' } borderRadius='12px' hasStripe isAnimated={true}/>
                    </Tooltip>
                </VStack>
            </VStack>
